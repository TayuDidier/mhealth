...
export default function ProviderMessages() {
  ...
  const myId = isDemoMode
    ? (SEED_PROVIDERS.find(p => p.email === profile?.email) || SEED_PROVIDERS[0])?.id
    : profile?.id
    ...
    if (isDemoMode) {
      ...
      setConversations(
        Object.values(grouped)
          .map(c => ({ ...c, partner: SEED_PATIENTS.find(p => p.id === c.partnerId) || { id: c.partnerId, name: 'Unknown' } }))
  ...
  // ── Patient list for search ───────────────────────────────────
  async function loadPatients() {
    if (isDemoMode) { setAllPatients(SEED_PATIENTS); return }
    const { data: users } = await supabase
      .from('users').select('id, name, email').eq('role', 'patient')
    if (!users?.length) { setAllPatients([]); return }
    const { data: profiles } = await supabase
      .from('patient_profiles').select('user_id, gestational_week')
      .in('user_id', users.map(u => u.id))
    setAllPatients(users.map(u => ({
      id: u.id,
      name: u.name,
      email: u.email,
  ...
  useEffect(() => { loadInbox(); loadPatients() }, [profile])
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`provider-chat-${selectedPartner.id}`)
  ...
  async function send(e) {
    e.preventDefault()
    if (!text.trim() || !online || !selectedPartner) return
    setSending(true)
    if (isDemoMode) {
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {p.gestational_week ? `Week ${p.gestational_week}` : p.email}
              </p>
            </div>
          </button>
        ))}
